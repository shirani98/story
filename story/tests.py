...
class SearchViewTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(alias='testuser',username='testuser', password='testpassword')
        self.tag = Tag.objects.create(name='testtag')
        self.story = Story.objects.create(body='testbody', brief='testbrief', user=self.user)
...
class StoryCreateAPIViewTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='testuser', password='testpassword', type='Author')
...
class StoryRetrieveUpdateDestroyAPIViewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.story.save()
        self.detail_url = reverse('story:story-detail', kwargs={'slug': self.story.slug})
        self.change_url = reverse('story:story-change', kwargs={'slug': self.story.slug})
        self.client = APIClient()
        self.client.force_authenticate(user=self.user)
