...
class ActivationAPIViewTestCase(APITestCase):
    def test_activation_success(self):
        user = User.objects.create(username='test_user',alias='test_alias',email='testuser@example.com')
    ...
    def test_activation_expired_token(self):
        token = jwt.encode({'user_id': 1}, settings.SECRET_KEY, algorithm='HS256').decode('utf-8')
        url = reverse('accounts:activation', kwargs={'token': token})
        response = self.client.get(url)
